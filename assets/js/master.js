...
function myDone(e) {
  let p = e.getAttribute('data-status')
  let t = e.nextElementSibling
  t.setAttribute('data-status', p)
...
let myLocal = JSON.parse(localStorage.getItem('database'))
...
if (myLocal != null) {
    ...
    <div id='div1' class="relative py-[10px] w-full overflow-hidden flex justify-between items-center group"> 
      ...
      <div id='div2' data-status="${val.attr}" class="relative w-[75%] md:w-[80%] h-full rounded-[10px] p-[10px] overflow-hidden shadow-xl bg-white shadow-slate-850 group after:absolute after:top-0 after:left-0 after:w-0 after:duration-[1s] after:h-full after:bg-[#3cec3caf]">
         ${val.txt}  
      </div>
          <div class="w-[15%] flex justify-end items-center">
              <div><i onclick="myEdit(this)" id="edit" class=" w-full cursor-pointer hover:text-[red] duration-[.4s] text-[#00235B] text-[7vw] md:text-[3.5vw] xl:text-[2vw] flex justify-end items-center h-full icon-pencil-circled"></i></div>
    ...
    let gncont1 = document.querySelectorAll('#div1>#div2')
    gncont1.forEach((val)=>{
    
      val.getAttribute('data-status')
      if (val.getAttribute('data-status') == "on") {
        val.classList.add('after:w-full')
        val.classList.remove('after:w-0')
        val.previousElementSibling.classList.add('w-[90px]')
        val.previousElementSibling.classList.remove('w-[57px]')
      }
...
function moon(){
  let gncont = document.querySelectorAll('#div1>#div2')
  gncont.forEach((val)=>{
    
    ff.push(val.innerText)
    
    const temp = {
      id: fag++,
      txt: val.innerText,
      attr: val.getAttribute('data-status')
       
    }
    dbc.push(temp)
  })

localStorage.setItem('database', JSON.stringify(dbc))
}
